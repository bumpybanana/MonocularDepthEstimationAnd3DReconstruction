...
class Transfire(nn.Module):
    def __init__(self,inplanes,s_plane,e_plane):
        ...
        self.squeeze_activation = nn.LeakyReLU(0.2)
        self.expand1x1_activation = nn.LeakyReLU(0.2)
        self.expand2x2_activation = nn.LeakyReLU(0.2)
    ...
    def forward(self, x, concat):
        ...
        x1 = F.interpolate(x1, size=[concat.size(2),concat.size(3)],mode='bilinear',align_corners=True)
        x2 = F.interpolate(x2, size=[concat.size(2),concat.size(3)], mode='bilinear', align_corners=True)
        #print(x.shape)
        x = torch.cat([
            x1, x2
        ], 1)
...
class Upsample(nn.Module): #two fire modules for upsampling
    ...
    def __init__(
        ...
        self.squeeze_activation = nn.LeakyReLU(0.2)
        self.expand3x3_activation = nn.LeakyReLU(0.2)
        self.expand1x1_activation = nn.LeakyReLU(0.2)
