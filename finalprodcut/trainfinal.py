...
from trainutils import AverageMeter,  colorize
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for i, sample_batched in enumerate(train_loader):
            ...
            # Compute the loss
            dx_true, dy_true = 8*kf.SpatialGradient()(depth_n)[:,:,0,:,:], 8*kf.SpatialGradient()(depth_n)[:,:,1,:,:]
            dx_pred, dy_pred = 8*kf.SpatialGradient()(output)[:,:,0,:,:], 8*kf.SpatialGradient()(output)[:,:,1,:,:]
            ...
            if epoch % 25 == 0:
                save_checkpoint(checkpoint, '{0}epochs0.1SSIM.pth.tar'.format(epoch))
        ...
        with torch.no_grad():
            for i, sampled_batch in enumerate(test_loader):
                ...
                # Compute the loss
                dx_true, dy_true = 8 * kf.SpatialGradient()(val_depth)[:, :, 0, :, :], 8 * kf.SpatialGradient()(val_depth)[
                                                                                         :, :, 1, :, :]
                dx_pred, dy_pred = 8 * kf.SpatialGradient()(val_output)[:, :, 0, :, :], 8 * kf.SpatialGradient()(val_output)[:,
                                                                                        :, 1, :, :]
                l_grad = torch.mean(torch.abs(dy_pred - dy_true) + torch.abs(dx_pred - dx_true))
                l_depth = l1_criterion(val_output, val_depth)
                l_ssim = torch.clamp((1 - ssim(val_output, val_depth, val_range=1 / 10)) * 0.5, 0, 1)

                val_loss = (1 * l_ssim) + (0.3 * l_depth) + (1 * l_grad)
        ...
        #save_checkpoint(checkpoint, '{epochs.pth.tar')



        # print examples to a folder
        save_predictions_dataset(
            test_loader, model, folder="saved_images_mobileval256_3/", device=device
        )
